    ...
    int frameCount)
{
    ...
    for (int frameIndex = 0; frameIndex < MAX_ANIMATION_FRAMES; frameIndex++)
        rangedCollision.anchor.hasAnchor[entityIndex][frameIndex] = false;

    for (int frameIndex = 0; frameIndex < frameCount; frameIndex++)
        rangedCollision.anchor.hasAnchor[entityIndex][frameIndex] =
            entityCollisionAnchor.hasAnchor[entityIndex][frameIndex];
}
