    ...
    int frameCount)
{
    ...
    for (int frameIndex = 0; frameIndex < MAX_ANIMATION_FRAMES; frameIndex++)
        rangedCollision.anchor.exists[entityIndex][frameIndex][0] = false;

    for (int frameIndex = 0; frameIndex < frameCount; frameIndex++)
        rangedCollision.anchor.exists[entityIndex][frameIndex][0] =
            entityCollisionAnchor.exists[entityIndex][frameIndex][0];
}
