...
void debugRenderSystem(const Context &ctx)
{
    ...
    renderColBox(ctx.renderer, entityColAABB(ctx.data.player), off, screen);

    for (uint32_t i = 0; i < ctx.data.npc.npcCount; i++)
    {
        renderColBox(ctx.renderer, entityColAABB(ctx.data.npc, i), off, screen);
    ...
    for (uint32_t i = 0; i < ctx.data.object.objectCount; i++)
        renderColBox(ctx.renderer, entityColAABB(ctx.data.object, i), off, screen);
